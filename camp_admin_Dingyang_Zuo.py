...
Press any other key to re-enter your selected date: ")
            reaction = str(input(""))
            if reaction == "1":
                disp_menu()
            else:
    ...
    #if date not exists in db_bookings, created a new tuple with date and None(to represent name, site, occupancy)
    target_list = []
...
def add_customer():
    ...
    #get user input of name/email/phone
    print("Hi, welcome to register your new customer account!")
    new_name = input("please enter a new name: ")
    new_email = str(input("please enter a new email: "))
    new_phone = str(input("please enter a new phone number: "))
    #store user inputs and display feedback to user
    new_l = {'name':str(new_name), 'email': str(new_email), 'phone': str(new_phone)}
...
def add_booking():
    ...
    while True:
        try:
            ...
            resp = int(input("Press '1' to go back to menu page, press any other key to re-enter your id. "))
            if resp == 1:
                disp_menu()
    ...
    #insert_1 (a tuple with same format in value of db_bookings), add into db_bookings
    if stay_result == False:
        ...
        #add booking into db_bookings by date(key of dict) and site identifier(element in tuples within lists in dict value)
        for i in list_1:#user input dates are empty, insert booking information directly(datas verified)
            if i not in db_bookings.keys():
                db_bookings[i] = [insert_1]
            else:#insert booking data in the correct list/tuples and save as updated db_booking for reference
                for i in list_1:
                    if i not in db_bookings.keys():
                        db_bookings[i] = [insert_1]
                    else:
                        if db_bookings[i][0][0][0] in list_UNPS_name:
                            if entered_site in list_UNPS_name:
            ...
            #print(updated_db_bookings)
        print(f'***Congratulation, your booking is successful! ***\n \
***below is your booking information: ***\n \
Name: {entered_name}, ID: {entered_id} \n \
Camp Site: {entered_site}, Occupants: {entered_occu} \n \
Check-in Date: {entered_year}/{entered_month}/{entered_day}, Staying Day(s): {entered_stay} day(s)\n')
